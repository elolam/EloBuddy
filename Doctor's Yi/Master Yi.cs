...
namespace Yi
{
    class Program
    {
        ...
        {
            if (ComboMenu["antiGap"].Cast<CheckBox>().CurrentValue && Q.IsReady() && args.Sender.Distance(_Player) < 300)
            {
                Q.Cast(args.Sender);
            }
        }
            ...
            if (target != null)
     	    {
                ...
                if (useR && R.IsReady() && (_Player.Position.CountEnemyChampionsInRange(675) >= MinR || Player.Instance.HealthPercent <= 30))
                {
                    R.Cast();
	    		}
        ...
        private static void AIHeroClient_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (useW && !_Player.IsRecalling() && !_Player.IsInShopRange() && _Player.CountEnemyChampionsInRange(425) >= 1)
            {
        ...
        public static float QDamage(Obj_AI_Base target)
        {
            return _Player.CalculateDamageOnUnit(target, DamageType.Physical,
                (float)(new[] { 0, 25, 60, 95, 130, 165 }[Q.Level] + 1.0f * _Player.FlatPhysicalDamageMod));
        }
        ...
        public static void KillSteal()
        {
            ...
            foreach (var target in EntityManager.Heroes.Enemies.Where(e => e.IsValidTarget(Q.Range) && !e.HasBuff("JudicatorIntervention") && !e.HasBuff("kindredrnodeathbuff") && !e.HasBuff("Undying Rage") && !e.IsDead && !e.IsZombie))
            {
                ...
                {
                    if (target.Health + target.AttackShield <= QDamage(target) + Player.Instance.GetAutoAttackDamage(target))
                    {
                        Q.Cast(target);
                    }
                }
