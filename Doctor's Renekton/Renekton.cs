...
namespace Renekton7
{
    internal class Program
    {
        ...
        private static void Ultimate()
        {
            ...
            if (useR && _Player.HealthPercent <= minR && _Player.Position.CountEnemyChampionsInRange(500) >= 1 && !Player.Instance.IsInShopRange())
            {
                R.Cast();
            }
			
            if (useR2 && !Player.Instance.IsInShopRange() && _Player.Position.CountEnemyChampionsInRange(450) >= minE)
            {
                R.Cast();
            }
        }
