...
namespace XinZhao7
{
    static class Program
    {
        ...
        private static void Ultimate()
        {
            ...
            if (useR && !Player.Instance.IsInShopRange() && _Player.Position.CountEnemyChampionsInRange(R.Range) >= minR)
            {
                R.Cast();
            }

            if (useR2 && _Player.HealthPercent <= mauR && _Player.Position.CountEnemyChampionsInRange(R.Range) >= 1 && !Player.Instance.IsInShopRange())
            {
                R.Cast();
            }
        }
