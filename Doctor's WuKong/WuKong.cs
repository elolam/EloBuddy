...
namespace Doctor_s_WuKong
{
    static class Program
    {
        ...
        private static void Ultimate()
        {
            ...
            if (target != null)
            {
                if (useR && R.IsReady() && _Player.Position.CountEnemyChampionsInRange(R.Range) >= minR && !RActive)
                {
                    R.Cast();
                }
				
                if (useR2 && R.IsReady() && !RActive && _Player.HealthPercent <= mauR && _Player.Position.CountEnemyChampionsInRange(R.Range) >= 1 && !Player.Instance.IsInShopRange())
                {
                    R.Cast();
                }
				
                if (autow && W.IsReady() && !RActive && _Player.HealthPercent <= mauW && _Player.Position.CountEnemyChampionsInRange(R.Range) >= 1 && !Player.Instance.IsInShopRange())
                {
                    W.Cast();
                }
