    ...
    internal class Program
    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Drawings["DrawRhit"].Cast<CheckBox>().CurrentValue && target != null && R.IsReady() && target.IsValidTarget(R.Range))
            {
                ...
                {
                    Vector2 ft = Drawing.WorldToScreen(_Player.Position);
                    DrawFont(Thm, "[R] Can Hit " + RPred.CastPosition.CountEnemyChampionsInRange(400), (float)(ft[0] - 90), (float)(ft[1] + 20), SharpDX.Color.Orange);
                }
            }
        }
        ...
        private static void Combo()
        {
            ...
            foreach (var target in EntityManager.Heroes.Enemies.Where(e => e.IsValidTarget(R.Range) && !e.IsDead))
            {
                ...
                {
                    var RPred = R.GetPrediction(target);
                    if (RPred.CastPosition.CountEnemyChampionsInRange(400) >= MinR && RPred.HitChance >= HitChance.High)
                    {
