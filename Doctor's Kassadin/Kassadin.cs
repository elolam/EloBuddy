    ...
    internal class Program
    {
        ...
        private static void Harass()
        {
            ...
            if (target != null)
            {
                ...
                if (useR && R.IsReady() && target.IsValidTarget(R.Range) && target.Position.CountEnemyChampionsInRange(R.Range) <= minR && _Player.HealthPercent >= Minhp && Player.Instance.GetBuffCount("RiftWalk") < minRs)
                {
                    if (turret)
                    {
                        if (!UnderTuret(target))
        ...
        private static void Dtc()
        {
            ...
            {
                if (Seraph.IsOwned() && Seraph.IsReady() && _Player.HealthPercent <= Misc["Hp"].Cast<Slider>().CurrentValue && _Player.Position.CountEnemyChampionsInRange(R.Range) >= 1)
                {
                    Seraph.Cast();
                }
            }
